...
export function Column({ status, title, tasks, onTaskClick }: ColumnProps) {
  ...
  const getStatusBorder = (status: TaskStatus) => {
    switch (status) {
      ...
      case "in_progress": return "border-green-500/20";
      case "review": return "border-purple-500/20";
      case "done": return "border-teal-500/20";
      case "blocked": return "border-red-500/20";
      default: return "border-border";
    }
  };

  return (
    <div className={`flex flex-col w-[320px] shrink-0 bg-background/50 border border-border/50 rounded-2xl ${getStatusBorder(status)} overflow-hidden`}>
